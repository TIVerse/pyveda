...
def _sometimes_fails(x: int) -> int:
    if x % 3 == 0 and x > 0:
        raise ValueError(f"Failed on {x}")
    return x**2


def _safe_task(x: int) -> int | None:
    try:
        return _sometimes_fails(x)
    except ValueError:
        return None
...
def _multi_error_task(x: int) -> int:
    if x % 3 == 0:
...
# Global state for error collection (not ideal but needed for testing)
_errors_collected = []


def _error_catching_wrapper(x: int) -> int | None:
    try:
        return _multi_error_task(x)
    except Exception as e:
        _errors_collected.append((x, type(e).__name__))
        return None


def _task_with_context(x: int) -> int:
    if x == 7:
...
def _flaky_task(x: int) -> int:
    if x not in _flaky_attempts:
        _flaky_attempts[x] = 0
    _flaky_attempts[x] += 1

    # Succeed on 2nd attempt
    if _flaky_attempts[x] < 2:
        raise ConnectionError("Temporary failure")
    return x * 2


def _retry_wrapper(x: int, max_retries: int = 3) -> int:
    for attempt in range(max_retries):
        try:
            return _flaky_task(x)
...
def _always_fails_connection(x: int) -> int:
    raise ConnectionError("Permanent failure")


def _retry_wrapper_exhausted(x: int, max_retries: int = 2) -> int | None:
    for attempt in range(max_retries):
        try:
            return _always_fails_connection(x)
        except ConnectionError:
            if attempt == max_retries - 1:
                return None
    return None
...
def _batch_processor(batch_id: int) -> list[int]:
    results = []
    for i in range(5):
        item = batch_id * 5 + i
...
def _mixed_success_task(x: int) -> tuple[int, bool]:
    success = x % 2 == 0
    return (x if success else -1, success)


def _primary_service(x: int) -> str:
    if x % 3 == 0:
        raise ConnectionError("Primary unavailable")
    return f"primary-{x}"


def _fallback_service(x: int) -> str:
    return f"fallback-{x}"


def _resilient_task(x: int) -> str:
    try:
        return _primary_service(x)
    except ConnectionError:
        return _fallback_service(x)


def _high_quality_task(x: int) -> tuple[int, str]:
    # Simulates expensive operation
    time.sleep(0.01)
    return (x * 100, "high")


def _low_quality_task(x: int) -> tuple[int, str]:
    # Fast but lower quality
    return (x * 10, "low")


# Global flag for quality (not ideal but needed for testing)
_use_high_quality = False


def _adaptive_task(x: int) -> tuple[int, str]:
    if _use_high_quality:
        return _high_quality_task(x)
    else:
        return _low_quality_task(x)


def _scope_failing_task(x: int) -> int:
    if x == 2:
        raise ValueError("Task 2 failed")
    return x * 2


def _scope_partial_task(x: int) -> int:
    if x == 3:
...
def _map_step1(x: int) -> int:
    return x * 2


def _map_step2(x: int) -> int:
    if x == 10:
        raise ValueError("Error in step2")
    return x + 1


def _filter_predicate(x: int) -> bool:
    if x == 5:
        raise ValueError("Filter error")
    return x % 2 == 0


def _fold_reducer(acc: int, x: int) -> int:
    if x == 7:
...
class TestFailurePolicies:
    ...
    def test_fail_fast_on_error(self):
        ...
        with pytest.raises(ValueError, match="Task 5 failed"):
            veda.par_iter(range(10)).map(_failing_task).collect()

    def test_continue_on_error_with_wrapper(self):
        """Test continuing execution with error wrapper."""
        result = veda.par_iter(range(10)).map(_safe_task).collect()
    ...
    def test_error_in_all_tasks(self):
        ...
        with pytest.raises(RuntimeError):
            veda.par_iter(range(5)).map(_always_fails).collect()
...
class TestErrorAggregation:
    ...
    def test_collect_multiple_error_types(self):
        ...
        global _errors_collected
        _errors_collected = []  # Reset global state

        veda.par_iter(range(15)).map(_error_catching_wrapper).collect()

        # Verify we caught errors
        assert len(_errors_collected) > 0
        assert any(err[1] == "ValueError" for err in _errors_collected)
        assert any(err[1] == "TypeError" for err in _errors_collected)

    def test_error_context_preservation(self):
        """Test that error context is preserved."""

        with pytest.raises(ValueError) as exc_info:
            veda.par_iter(range(10)).map(_task_with_context).collect()
...
class TestRetryLogic:
    ...
    def test_retry_succeeds_eventually(self):
        ...
        _flaky_attempts = {}  # Reset global state

        result = veda.par_iter(range(5)).map(_retry_wrapper).collect()
    ...
    def test_retry_exhausted(self):
        ...
        result = veda.par_iter(range(3)).map(_retry_wrapper_exhausted).collect()
...
class TestPartialResults:
    ...
    def test_recover_partial_batch_results(self):
        ...
        batches = veda.par_iter(range(4)).map(_batch_processor).collect()
    ...
    def test_partial_results_with_mixed_success(self):
        ...
        results = veda.par_iter(range(10)).map(_mixed_success_task).collect()
...
class TestGracefulDegradation:
    ...
    def test_fallback_on_primary_failure(self):
        ...
        results = veda.par_iter(range(9)).map(_resilient_task).collect()
    ...
    def test_quality_degradation(self):
        ...
        global _use_high_quality
        _use_high_quality = False  # Simulate high load

        results = veda.par_iter(range(5)).map(_adaptive_task).collect()
...
class TestScopeErrorHandling:
    ...
    def test_scope_error_propagation(self):
        ...
        with pytest.raises(ValueError, match="Task 2 failed"):
            with veda.scope() as s:
                [s.spawn(_scope_failing_task, i) for i in range(5)]
                s.wait_all()
    ...
    def test_scope_partial_completion(self):
        ...
        with pytest.raises(RuntimeError):
            with veda.scope() as s:
                [s.spawn(_scope_partial_task, i) for i in range(5)]
                s.wait_all()

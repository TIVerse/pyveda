...
class TestErrorAggregation:
    ...
    def test_collect_multiple_error_types(self):
        ...
        def error_catching_wrapper(x: int) -> Optional[int]:
            try:
        ...
        result = veda.par_iter(range(15)).map(error_catching_wrapper).collect()
...
class TestRetryLogic:
    ...
    def test_retry_exhausted(self):
        ...
        def retry_wrapper(x: int, max_retries: int = 2) -> Optional[int]:
            for attempt in range(max_retries):
                try:
...
class TestScopeErrorHandling:
    ...
    def test_scope_error_propagation(self):
        ...
        with pytest.raises(ValueError, match="Task 2 failed"):
            with veda.scope() as s:
                futures = [s.spawn(failing_task, i) for i in range(5)]
                results = s.wait_all()
    ...
    def test_scope_partial_completion(self):
        ...
        with pytest.raises(RuntimeError):
            with veda.scope() as s:
                futures = [s.spawn(task, i) for i in range(5)]
                results = s.wait_all()

...
class TestCompleteWorkflows:
    ...
    def test_etl_pipeline(self):
        ...
        veda.par_iter(transformed).map(load).collect()
    ...
    def test_deterministic_workflow(self):
        ...
        # Note: Full determinism with random in multiprocessing is complex
        # This test verifies that execution order is consistent

        def compute_task(x: int) -> int:
            # Deterministic computation (no randomness)
            return (x * x + x * 2 + 1) % 100

        # Run 1
        with veda.deterministic(seed=42):
            result1 = veda.par_iter(range(50)).map(compute_task).collect()

        # Run 2 with same seed - should preserve order
        with veda.deterministic(seed=42):
            result2 = veda.par_iter(range(50)).map(compute_task).collect()

        # Results should be identical (deterministic function + same seed)
        assert result1 == result2

        # Both should equal the expected deterministic result
        expected = [compute_task(i) for i in range(50)]
    ...
    def test_nested_scopes(self):
        ...
        def outer_task(x: int) -> int:
            ...
            with veda.scope() as inner_scope:
                inner_scope.spawn(lambda: x * 2)
                inner_scope.spawn(lambda: x * 3)
        ...
        with veda.scope() as outer_scope:
            [outer_scope.spawn(outer_task, i) for i in range(5)]
...
class TestConfigurationWorkflows:
    ...
    def test_thread_only_config(self):
        ...
        veda.Config.builder().policy(veda.SchedulingPolicy.THREAD_ONLY).build()
    ...
    def test_custom_worker_count(self):
        ...
        config = veda.Config.builder().threads(4).processes(2).build()
...
class TestTelemetryIntegration:
    ...
    def test_metrics_collection(self):
        ...
        if importlib.util.find_spec("vedart.telemetry.metrics") is None:
...
class TestLargeScaleWorkflows:
    ...
    def test_memory_efficient_processing(self):
        ...
        # Process 1000 chunks
        results = (
            veda.par_iter(range(1000)).map(create_data).map(process_data).collect()
        )
...
class TestChainedOperations:
    ...
    def test_map_with_aggregation(self):
        ...
        veda.par_iter(range(50)).map(lambda x: x**2)
